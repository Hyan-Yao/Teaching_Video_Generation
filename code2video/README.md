...
Provide your **API credentials**. `api_config.json` is a committed template that holds
non-secret defaults (base URLs, model names) — **do not put real keys there.**

Keep secrets in a local, git-ignored `.env` file instead:

```bash
cp .env.example .env   # then fill in your keys
```

Each variable follows the `{SERVICE}_{KEY}` (uppercase) pattern and overrides the
matching entry in `api_config.json` at runtime — e.g. `GPT5_API_KEY` overrides
`gpt5.api_key`. See `.env.example` for the full list.
