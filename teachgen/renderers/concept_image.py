...
class ConceptImageRenderer:
    ...
    def render(self, seg: Segment, ctx: RenderContext) -> VisualAsset:
        ...
        image_prompt = ctx.provider.chat(
            ci.build_user_brief(
                seg.visual_brief, ctx.cfg.audience, f"{_STYLE}; {theme.concept_style()}"
            ),
            system=ci.build_prompt_system(theme.concept_constraint()),
            max_tokens=800,
        )
        image_prompt = f"{image_prompt.rstrip()}\n\n{theme.concept_constraint()}"
